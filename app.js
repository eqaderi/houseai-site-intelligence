(() => {
  ...
  function statusClass(status) {
    ...
    if (["preliminary-inference", "preliminary-engineering-inference", "regional-data"].includes(status)) return "preliminary";
    return "unresolved";
  }
      ...
      .map(
        (polygon) => `
          <article class="polygon-card">
            ...
            <h3>${escapeHtml(t("polygonLabel"))} ${escapeHtml(polygon.id)}</h3>
            <p>${escapeHtml(polygon.point_order.join(" → "))}</p>
            <dl>
  ...
  function updateSolarReadout() {
    ...
    $$("[data-solar-time-label]").forEach((element) => {
      element.textContent = localizeDigits(decimalHour(position.clock_hour));
    });
  ...
  function syncSolarSeasonButtons() {
    $$("[data-solar-season]").forEach((button) => {
      button.classList.toggle("active", button.dataset.solarSeason === state.solarSeason);
    });
  }
  ...
  function renderWindEvidence() {
    $("#wind-season-tabs").innerHTML = data.wind.seasons
      .map((season) => `
        <button class="${season.season === state.windSeason ? "active" : ""}" type="button" data-wind-season="${escapeHtml(season.season)}">${escapeHtml(seasonLabel(season.season))}</button>`)
      .join("");
    ...
    $("#wind-stats").innerHTML = `
      <div><span>${escapeHtml(t("prevailing"))}</span><strong>${escapeHtml(compass(season.prevailing_direction))}</strong></div>
      <div><span>${escapeHtml(t("meanSpeed"))}</span><strong><bdi>${format(season.mean_speed_ms, 1)} m/s</bdi></strong></div>
      <div><span>P90</span><strong><bdi>${format(season.p90_speed_ms, 1)} m/s</bdi></strong></div>
  ...
  function renderConcepts() {
    ...
    $$("[data-concept-option]").forEach((button) => {
      button.classList.toggle("active", button.dataset.conceptOption === state.conceptOption);
    });
  ...
  function renderDerivedStatistics() {
    ...
    if (derived) {
      ...
      $("#climate-derived").innerHTML = [
        metricRow(`${t("hdd")} 18°C`, format(byId.get("hdd18").annual_k_day, 0), "K·day"),
        metricRow(`${t("cdd")} 10°C`, format(byId.get("cdd10").annual_k_day, 0), "K·day"),
  ...
  function renderSpecies() {
    ...
    $("#species-grid").innerHTML = register.species
      .map((species) => {
        ...
        return `
        <article class="species-card${hidden ? " hidden" : ""}" data-verdict="${escapeHtml(species.verdict)}">
          ${photo(species)}
          <header>
  ...
  function renderInvestigations() {
    ...
    $("#investigation-gates").innerHTML = gates
      .map((gate) => `
        <button class="${gate.id === state.investigationGate ? "active" : ""}" type="button" data-investigation-gate="${escapeHtml(gate.id)}">${escapeHtml(local(gate.label))}</button>`)
      .join("");
    ...
    $("#investigations-table tbody").innerHTML = rows
      .map((item) => `
        <tr>
          ...
          <td>${escapeHtml(local(item.procure_via))}</td>
  ...
  function drawGeography() {
    ...
    [0.25, 1, 5].forEach((radiusKm, index) => {
      context.beginPath();
      context.arc(siteX, siteY, radiusKm / kmPerPixel, 0, Math.PI * 2);
      context.strokeStyle = index === 2 ? "rgba(189,109,77,0.45)" : "rgba(189,109,77,0.25)";
      context.lineWidth = index === 2 ? 1.5 : 1;
      context.setLineDash(index === 2 ? [5, 5] : [2, 4]);
      context.stroke();
    });
    context.setLineDash([]);
  ...
  function bindEvents() {
    ...
    $$("[data-climate-view]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        $$("[data-climate-view]").forEach((item) => item.classList.toggle("active", item === button));
        renderClimateEvidence();
        drawClimate();
      });
    });
    ...
    $$("[data-solar-object]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        $$("[data-solar-object]").forEach((item) => item.classList.toggle("active", item === button));
    ...
    $$("[data-hazard-filter]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        $$("[data-hazard-filter]").forEach((item) => item.classList.toggle("active", item === button));
        renderHazards();
      });
    });
    ...
    $$("[data-species-filter]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        $$("[data-species-filter]").forEach((item) => item.classList.toggle("active", item === button));
        renderSpecies();
      });
    });

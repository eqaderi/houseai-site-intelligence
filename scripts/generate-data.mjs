...
const project = {
  ...
  subtitle: bi(
    "Verified site geometry with traceable mountain-climate, solar, wind and hazard context",
    "هندسه تأییدشده سایت با زمینه قابل‌ردیابی اقلیم کوهستانی، خورشید، باد و مخاطرات",
  ),
...
const documents = {
  items: [
    ["unified-top", "Unified site top view", "نمای بالای سایت یکپارچه", "Neutral plan showing the seven-point outer boundary, Pt8 terrain hub, road edge, north and study zones.", "پلان خنثی شامل مرز هفت‌نقطه‌ای، نقطه داخلی Pt8، لبه راه، شمال و محدوده‌های مطالعاتی.", "PNG", "neutral", "phase-1", "verified", "assets/diagrams/unified-site-top-view.png", "image"],
    ...
    ["summary-fa", "Verified site summary — Persian", "خلاصه تأییدشده سایت — فارسی", "Persian summary generated only from verified site values and explicit limitations.", "خلاصه فارسی تولیدشده صرفاً از مقادیر تأییدشده و محدودیت‌های صریح.", "MD", "fa", "phase-4", "verified-derived", "assets/documents/site-summary.fa.md", "document"],
    ...
    ["unified-fcstd", "Unified FreeCAD model", "مدل یکپارچه FreeCAD", "Validated unified boundary, TIN, contours, sections and study zones.", "مرز یکپارچه، TIN، خطوط تراز، مقاطع و محدوده‌های مطالعاتی تأییدشده.", "FCStd", "neutral", "phase-2", "verified-derived", "assets/documents/unified-site.FCStd", "model"],
    ...
    ["boundaries-dxf", "Validated site boundaries", "مرزهای تأییدشده سایت", "ASCII R2013 DXF containing 3D boundaries and survey points.", "فایل DXF نوع ASCII R2013 شامل مرزهای سه‌بعدی و نقاط نقشه‌برداری.", "DXF", "neutral", "phase-1", "verified", "assets/documents/site-boundaries.dxf", "model"],
...
const translationsEn = {
  ...
  overviewLead: "The three source polygons have been unified without changing their verified combined area. Pt8 is retained only as an interior terrain point.",
  ...
  surveyTitle: "From three survey polygons to one verified property",
  surveyLead: "The boundary story remains auditable: source polygons, point associations, side lengths, area checks and cryptographic hashes are all retained.",
  originalPolygons: "Original polygons",
  unifiedEdges: "Unified outer edges",
  ...
  webglUnavailable: "Interactive 3D is unavailable in this browser. The verified 2D terrain plan remains available below.",
  ...
  ariaHeroBoundary: "Animated real property boundary",
  ariaBoundaryPlan: "North-up unified seven-point property boundary with Pt8 interior terrain point and southern road edge",
  ...
  ariaTerrainMap: "Interactive north-up terrain view with unified property boundary, TIN contours and measured elevations",
...
const translationsFa = {
  ...
  overviewLead: "سه چندضلعی منبع بدون تغییر مساحت مجموع تأییدشده، یکپارچه شده‌اند. Pt8 فقط به‌عنوان نقطه داخلی زمین حفظ شده است.",
  ...
  surveyTitle: "از سه چندضلعی نقشه‌برداری تا یک ملک تأییدشده",
  surveyLead: "روایت مرز قابل ممیزی است: چندضلعی‌ها، ارتباط نقاط، طول اضلاع، کنترل مساحت و هش‌های رمزنگاری حفظ شده‌اند.",
  originalPolygons: "چندضلعی‌های اولیه",
  unifiedEdges: "اضلاع بیرونی یکپارچه",
  ...
  webglUnavailable: "نمای سه‌بعدی تعاملی در این مرورگر در دسترس نیست. پلان دوبعدی تأییدشده در پایین باقی مانده است.",
  ...
  ariaHeroBoundary: "نمایش متحرک مرز واقعی ملک",
  ariaBoundaryPlan: "مرز یکپارچه هفت‌نقطه‌ای ملک با شمال به بالا، نقطه داخلی زمین Pt8 و لبه راه جنوبی",
  ...
  ariaTerrainMap: "نمای تعاملی زمین با شمال به بالا، مرز یکپارچه ملک، خطوط تراز TIN و ترازهای اندازه‌گیری‌شده",
...
const investigations = {
  ...
  items: [
    ...
    investigationRow("title-boundary", "legal", "before-permit",
      ...
      "Any boundary-dependent setback, party wall or right of way",
      "هر عقب‌نشینی، دیوار مشترک یا حق ارتفاق وابسته به مرز",
      "A verified 487.428568 m² geometry derived from the survey drawing, which is a measurement rather than a title",
      "هندسه تأییدشده ۴۸۷٫۴۲۸۵۶۸ مترمربع برگرفته از نقشه برداشت، که اندازه‌گیری است نه سند"),
    ...
    investigationRow("road-gradient", "utilities", "before-concept",
      ...
      "A verified 10.270569 m road boundary length and its two end elevations, which give the edge but not the road's own gradient",
      "طول تأییدشده ۱۰٫۲۷۰۵۶۹ متر مرز راه و تراز دو انتهای آن، که لبه را می‌دهد اما شیب خود راه را نه"),
    investigationRow("construction-access", "utilities", "before-construction",
      ...
      "The verified geometry shows the constraint but does not resolve it",
      "هندسه تأییدشده محدودیت را نشان می‌دهد اما آن را حل نمی‌کند"),
...
fs.writeFileSync(
  ...
  `# خلاصه تأییدشده سایت\n\n` +
    `> این خلاصه فقط از نسخه تغییرناپذیر \`${version.site_version}\` تولید شده و توصیه پلان نهایی یا تأیید مهندسی نیست.\n\n` +
    `- مساحت پلان تأییدشده: **${version.site_area_m2.toFixed(6)} مترمربع**\n` +
    ...
    `## محدودیت‌های مهم\n\n` +
    `مدل زمین فقط بر هشت تراز نقطه‌ای استوار است و شکست‌خط، دیوار، جدول، آب زیرزمینی و تغییرات موضعی شیب را نشان نمی‌دهد. تفسیر CRS با موقعیت بانه‌ورده انطباق قوی دارد، اما هنوز توسط نقشه‌بردار و نقطه کنترل تأیید نشده است. داده‌های اقلیم، خورشید، باد، زمین‌شناسی، خاک و زلزله منطقه‌ای‌اند و جایگزین ایستگاه محلی، گمانه ژئوتکنیک، افق‌برداری یا طراحی مهندسی نیستند.\n`,
);
